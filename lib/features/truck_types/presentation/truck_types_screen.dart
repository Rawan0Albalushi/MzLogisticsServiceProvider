...
class TruckTypesScreen extends ConsumerWidget {
        ...
        children: [
          ...
          FilterBar(
            children: [
              FilterSearchField(
                onChanged: (value) =>
                    ref.read(truckTypeSearchProvider.notifier).state = value,
              ),
              FilterSelect(
                options: const ['active', 'inactive'],
                value: ref.watch(truckTypeStatusProvider),
                onChanged: (value) =>
                    ref.read(truckTypeStatusProvider.notifier).state = value,
              ...
              FilterSelect(
                ...
                onChanged: (value) =>
                    ref.read(truckTypeSourceProvider.notifier).state = value,
                labelOf: (value) => value == 'platform'
                    ? context.tr('truckTypes.platform')
                    : context.tr('truckTypes.company'),
            ...
            builder: (data) {
              ...
              final items = data.where((item) {
                if (search.isNotEmpty) {
                  final haystack = '${item.name} ${item.nameAr} ${item.code}'
                      .toLowerCase();
                        ...
                        ? Wrap(
                            ...
                            children: [
                              ...
                              TextButton(
                                ...
                                child: Text(
                                  item.isActive
                                      ? context.tr('truckTypes.disable')
                                      : context.tr('truckTypes.enable'),
                                ),
                              ),
                              if (!item.isSystem)
                                TextButton(
  ...
  Future<void> _toggle(
    ...
    WidgetRef ref,
    TruckTypeOption item,
  ) async {
    if (item.id == null) {
      return;
    }
    try {
      await ref.read(truckTypeRepositoryProvider).update(item.id!, {
    ...
    await showDialog<void>(
      ...
      builder: (context) {
        return AlertDialog(
          title: Text(
            item == null
                ? context.tr('truckTypes.add')
                : context.tr('truckTypes.edit'),
          ),
          content: Form(
            key: formKey,
            child: SizedBox(
              ...
              onPressed: () async {
                ...
                try {
                  ...
                  if (item == null) {
                    ...
                    await ref
                        .read(truckTypeRepositoryProvider)
